...
function findRecipesByIngredients() {
    ...
    const selectedIngredients = Array.from(document.querySelectorAll('.ingredient-options input[type="checkbox"]:checked'))
    ...
    // Helper function to check if recipe contains any of the selected ingredients
    function recipeContainsIngredient(recipe) {
...
function displayMatchingRecipes(matchingRecipes) {
    ...
    if (matchingRecipes.length === 0) {
        ...
        const resultsHTML = `
            <div class="modal-content">
                <span class="close">&times;</span>
                <h2>Matching Recipes (${matchingRecipes.length} found)</h2>
                <div class="recipe-grid">
                    ${matchingRecipes.map(recipe => `
                        <div class="recipe-card" onclick='showRecipeDetails(${JSON.stringify(recipe)})'>
                            <img src="${recipe.image}" alt="${recipe.title}">
                            <h3>${recipe.title}</h3>
                            <p class="recipe-category">${recipe.category.charAt(0).toUpperCase() + recipe.category.slice(1)}</p>
                            <p>Prep Time: ${recipe.prepTime}</p>
                            <p>Calories: ${recipe.calories}</p>
                            <p class="matching-ingredients">${recipe.matchingIngredients} matching ingredient${recipe.matchingIngredients > 1 ? 's' : ''}</p>
                        </div>
                    `).join('')}
                </div>
            </div>
...
// Add event listener for the find recipes button
document.addEventListener('DOMContentLoaded', () => {
    const findRecipesButton = document.getElementById('findRecipes');
    if (findRecipesButton) {
        findRecipesButton.addEventListener('click', findRecipesByIngredients);
    }
});